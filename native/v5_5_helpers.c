...
typedef struct {
  ...
  u32 white_pawn_files,black_pawn_files,pawn_eval_packed;
...
static void search_refresh_pawn_eval(SearchState *s){
  int mg=0,eg=0;
  ...
  s->pawn_eval_packed=(u32)(u16)(short)mg|((u32)(u16)(short)eg<<16);
}
static int search_pawn_mg(const SearchState *s){return (int)(short)(s->pawn_eval_packed&65535u);}
static int search_pawn_eg(const SearchState *s){return (int)(short)(s->pawn_eval_packed>>16);}
...
static void search_apply_child(const SearchState *parent,SearchState *s,u32 m,SearchBoardUndo *u){
  *s=*parent;
  ...
  if(absolute(moving)==1||absolute(moving)==6||absolute(u->captured)==1)search_refresh_pawn_eval(s);
  s->side=-s->side;
  s->hash^=search_meta_token(s->side,s->castling,s->ep);
}
...
static int search_evaluate_state(const SearchState *s){
  int mg=s->eval_mg+search_pawn_mg(s),eg=s->eval_eg+search_pawn_eg(s),phase=s->eval_phase;
...
int search_all(int side,int castling,int ep,int wk,int bk,int halfmove,
  ...
  s.hash=search_initial_hash(&s);search_initial_eval(&s);search_refresh_pawn_eval(&s);search_init_packed_board();
