...
/* When the side to move is not in check, a non-king move from an unpinned
   square cannot expose its king. Identify the few pinned pieces once per
   generation, then keep the original make-and-attack verifier only for king
   moves, pinned pieces, en passant and checked positions. */
static u64 search_pinned_mask(int side,int king){
  u64 pinned=0;
  for(int i=0;i<16;i+=2){
    int df=dirs[i],dr=dirs[i+1],f=(king&7)+df,r=(king>>3)+dr,candidate=-1;
    while(f>=0&&f<8&&r>=0&&r<8){
      int sq=r*8+f,p=board[sq];
      if(p){
        int piece_side=p>0?1:-1;
        if(piece_side==side){
          ...
          if(candidate>=0){
            int type=absolute(p);
            if(type==5||(i<8?type==3:type==4))pinned|=(u64)1<<candidate;
          }
          break;
        }
      }
...
static int search_generate(const SearchState *s,int mode,int check){
  ...
  gen_side=side;gen_king=king;gen_mode=mode;gen_count=0;
  search_gen_check=check;
  search_gen_pinned=check?0:search_pinned_mask(side,king);
