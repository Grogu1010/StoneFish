...
typedef struct {
  ...
  u32 hash;
  int eval_mg,eval_eg,eval_phase,white_bishops,black_bishops,pawn_mg,pawn_eg;
...
static int search_evaluate_state(const SearchState *s){
  int mg=s->eval_mg+s->pawn_mg,eg=s->eval_eg+s->pawn_eg,phase=s->eval_phase;
