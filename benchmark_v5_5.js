...
vm.runInThisContext(loadedSources.map(entry => entry.source).join('\n\n'), {
  filename: 'stonefish-v5-5-bundle.js'
});
...
function assertContract() {
  if (STONEFISH_V5_5.knowledgeBase !== 'Native tapered positional evaluation') {
    throw new Error(`Wrong v5.5 knowledge base: ${STONEFISH_V5_5.knowledgeBase}`);
  }
  if (STONEFISH_V5_5.search !== 'Native PVS') {
    throw new Error(`Wrong v5.5 search core: ${STONEFISH_V5_5.search}`);
  }
...
function assertNoARMXControlWorks() {
  ...
  const game = new Chess();
  const move = withSeed(0x5150, () => getStonefishV55NoARMXMove(game));
...
function assertARMXResetsByGame() {
  ...
  const first = stonefishV55HostSearch(gameA).finished.slice(0, 2);
  const second = stonefishV55HostSearch(gameB).finished.slice(0, 2);
...
function latencyAndBehavior(samples) {
  ...
  if (samples.length) {
    ...
    clearSharedEngineCaches();
    withSeed(0x7702, () => getStonefishV55NoARMXMove(cloneGame(samples[0])));
    clearSharedEngineCaches();
    withSeed(0x7703, () => getStonefishV55Move(cloneGame(samples[0])));
    clearSharedEngineCaches();
  }
  ...
  for (let i = 0; i < samples.length; i += 1) {
    ...
    const noArmxResult = timedMove(seed, () => getStonefishV55NoARMXMove(cloneGame(samples[i])));
    const armxResult = timedMove(seed, () => getStonefishV55Move(cloneGame(samples[i])));
...
const definitions = {
  armxVsPro: ['ARMX-vs-Pro', getStonefishV55Move, getStonefishV5ProMove],
  noArmxVsPro: ['NoARMX-vs-Pro', getStonefishV55NoARMXMove, getStonefishV5ProMove],
  armxVsNoArmx: ['ARMX-vs-NoARMX', getStonefishV55Move, getStonefishV55NoARMXMove]
};
...
if (process.env.COMPARE_BASELINE === '1' || process.env.MATCHUP === 'armxVsBaseline' || process.env.RELEASE_GATE === '1') {
  ...
  if (process.env.EXACT_BEHAVIOR_PARITY === '1') {
    ...
    for (const row of fixtures) {
      ...
      const current = summarize(stonefishV55ScoreAllMoves(game), SF55C_LAST);
    ...
    definitions.armxVsBaseline = ['ARMX-vs-FrozenARMX', getStonefishV55Move, baselineMove];
  }
}
...
const result = {
  ...
  model: STONEFISH_V5_5.name,
  knowledgeBase: STONEFISH_V5_5.knowledgeBase,
  search: SF55C,
...
console.log('\nSTONEFISH_V5_5 ' + JSON.stringify(result));
