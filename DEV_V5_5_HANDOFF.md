...
The user explicitly requires **reset notes after every game**, even against the same opponent. Quiet-choice weights begin at zero; after four voluntary quiet choices, a frozen per-search policy prioritizes opponent quiet replies and permits reduced null-window probes for disfavored replies. Full verification follows if the probe favors the opponent. No ARMX passes no policy. Shared evaluation, node budget and maximum depth are unchanged. The 128-position contract in `benchmark_armx_reply_policy.js` checks exact control/prototype moves, scores, depth, nodes, learned weights and state restoration. Per-round isolation also remains covered by `benchmark_armx_contract.js`.
