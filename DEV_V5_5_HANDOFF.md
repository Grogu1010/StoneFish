...
Latest ARMX calibration: 47W-43L-10D /100 vs No ARMX (up from 42W-44L-14D), 90W-8L-2D /100 vs Pro, 4.702x faster than Pro. The frozen No-ARMX native host reproduced 92W-7L-1D /100 vs Pro. User explicitly requested focusing only on ARMX now and supplied a 10,000-game browser baseline corroborating the old ARMX deficit. The only unmet gate is 65 actual wins /100 against No ARMX. Keep native engine fixed, improve genuine per-game learning, commit verified gains directly to main, and do not release prematurely. Detailed evidence and experiment status are in DEV_V5_5_TESTUNIT1.md.

The user explicitly confirmed **reset notes after every game**. A fresh 100-game set (`START_INDEX=100`, 50 mirrored openings with no overlap) scored 48W-44L-8D against No ARMX. Combined calibrated results are 95W-87L-18D /200. Twenty-one rejected variants and 1,020 experimental games are archived with exact experimental sources in `benchmarks/v5_5/armx-rejected-experiments.json.gz`; full holdout evidence is `armx-calibrated-holdout-100.json.gz`. No new engine variant beat the calibration in confirmation. Do not re-promote these failed experiments without new evidence. Per-round isolation is explicitly covered by `benchmark_armx_contract.js`.

Read DEV_V5_5_TESTUNIT1.md first. It supersedes the architecture and "latest" results below. Both variants use the same frozen Stonefish_v5_5_native.js host, with ARMX-preview as the only difference. The current engine sources remain those of commit eda5960077a5410f5a7f72807b5105e9c1626700. All release targets remain mandatory; the ARMX contribution target is not met. No blog post requested.
