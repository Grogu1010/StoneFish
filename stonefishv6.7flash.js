(function () {
  ...
  const DRAW_WHILE_LOSING = 820000;
  const MAX_CACHE_SIZE = 90000;
  const FLASH_REPLY_WIDTH = 4;
  ...
  function v67FlashTempoBook(game, legal, color) {
    const history = game.moveHistory || [];
    if (history.length === 0 && color === "w") return bookMoveByUci(game, legal, "c2c4");
    if (history.length === 1 && color === "b" && history[0].from === "g1" && history[0].to === "f3") return bookMoveByUci(game, legal, "c7c5");
    return null;
  }

  function shouldUseAntiNf3FullLane(game, color) {
    const history = game.moveHistory || [];
    return color === "b" &&
      history.length > 1 &&
      history.length <= 20 &&
  ...
  window.StonefishV67Flash = {
    ...
    chooseMove(game, color) {
      ...
      for (let i = 0; i < finalists.length; i++) {
        ...
        if (mustPlan) {
          ...
          candidate.score -= opponentPlan.worstNetPlan * 0.80;
          candidate.score -= opponentPlan.totalNetPlan * 0.08;
          candidate.score += Math.max(0, currentPlan.worstNetPlan - opponentPlan.worstNetPlan) * 0.76;
    ...
    _stonefishDepthProfile: "v6.7 Flash architecture: v6 ply profile, candidate-narrowed tactical verification, cached fast path, opponent-plan prevention, and book-safe conversion tuning"
  };
})();
