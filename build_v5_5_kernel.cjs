...
try {
 ...
 const args=directCC
  ?['--target=wasm32','-O3','-flto','-msimd128','-mbulk-memory','-nostdlib','-Wl,--no-entry',...exports.map(name=>'-Wl,--export='+name),'-Wl,--export-memory',source,'-o',output]
  :['cc','-target','wasm32-freestanding','-O3','-flto','-msimd128','-mbulk-memory','-nostdlib','-Wl,--no-entry',...exports.map(name=>'-Wl,--export='+name),'-Wl,--export-memory',source,'-o',output];
