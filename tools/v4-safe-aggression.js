...
source=source.replace("function feature(g,m,c){",String.raw`function fourPlyScore(g,root){
  ...
  for(const reply of replies){
    ...
    for(const response of responses){
      const ourGain=V[response.captured]||0;g.fastApply(response);const fourth=g.fastMoves();let score;
      if(!fourth.length&&g.in_check())score=M;else{let fourthGain=0,fourthMate=false;for(const fm of fourth){const cap=V[fm.captured]||0;if(cap>fourthGain)fourthGain=cap;if(!fourthMate&&g.fastGivesCheck(fm)){g.fastApply(fm);if(g.fastMoves().length===0&&g.in_check())fourthMate=true;g.fastUndo();}}score=fourthMate?-M:(immediate-oppGain+ourGain-fourthGain);}
...
function feature(g,m,c){if(c==='fourPly')return fourPlyScore(g,m);if(c==='captureNow')return V[m.captured]||0;`);
...
const tail=String.raw`const C=[];const base=${JSON.stringify(base)};function add(name,e,extras){const o=base.slice();o.splice(5,0,'fourPly',...extras);C.push({name,order:o,developUntil:8,queenUntil:14,endgameMaterial:e});}
for(const e of[10,14]){add('fp_e'+e,e,[]);add('fp_check_e'+e,e,['check']);add('fp_cap_e'+e,e,['captureNow']);add('fp_check_cap_e'+e,e,['check','captureNow']);add('fp_cap_check_e'+e,e,['captureNow','check']);}
console.log('SAFE AGGRESSION SCREEN',C.length,'x 8');const s=C.map(c=>({cfg:c,r:test(c,8,261000)})).sort(rank);for(const x of s)console.log(x.cfg.name,x.r);
console.log('SAFE AGGRESSION CONFIRM top 5 x 30');const c=s.slice(0,5).map(x=>({cfg:x.cfg,r:test(x.cfg,30,264000)})).sort(rank);for(const x of c)console.log(x.cfg.name,x.r);
console.log('SAFE AGGRESSION FINAL top 2 x 100');const f=c.slice(0,2).map(x=>({cfg:x.cfg,r:test(x.cfg,100,270000)})).sort(rank);for(const x of f)console.log('FINAL',x.cfg.name,x.r);console.log('WINNER',JSON.stringify(f[0].cfg),f[0].r);`;
