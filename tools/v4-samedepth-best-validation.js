...
const control={name:'control',order:['promotion','castleNow','openingDevelop','hangingMax','mobility','queenDiscipline','repetitionAvoid','fiftyReset','endgamePawnProgress','endgameCheck','pieceSupport','kingFreedom','center','minorCentral','kingProtection','pawnStructure','rookActivity','kingPlacement','boardControl']};
const best={name:'best',order:['promotion','rep2Lead2_5','castleNow','openingDevelop','hangingMax','cycleProgress2_5','mobility','queenDiscipline','repetitionAvoid','fiftyReset','endgamePawnProgress','endgameCheck','pieceSupport','kingFreedom','center','minorCentral','kingProtection','pawnStructure','rookActivity','kingPlacement','boardControl']};
const starts=[760000,780000,800000,840000];
for(const cfg of[control,best]){let W=0,L=0,D=0;console.log('CFG',cfg.name);for(const st of starts){const r=test(cfg,100,st);W+=r.W;L+=r.L;D+=r.D;console.log(' BLOCK',st,r);}console.log('TOTAL',cfg.name,{W,L,D,games:400,score:W-8*L+.01*D});}
`;
