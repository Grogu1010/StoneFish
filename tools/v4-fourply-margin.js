...
const tail=String.raw`const C=[];const base=${JSON.stringify(base)};for(const m of[0,0.5,1,2,3,4,5,8]){const o=base.slice();o.splice(5,0,'fourPlyMargin:'+m);C.push({name:'m'+m,order:o,developUntil:8,queenUntil:14,endgameMaterial:14});}
console.log('FOURPLY MARGIN SCREEN',C.length,'x 8');const s=C.map(c=>({cfg:c,r:test(c,8,281000)})).sort(rank);for(const x of s)console.log(x.cfg.name,x.r);
console.log('FOURPLY MARGIN CONFIRM top 5 x 30');const c=s.slice(0,5).map(x=>({cfg:x.cfg,r:test(x.cfg,30,284000)})).sort(rank);for(const x of c)console.log(x.cfg.name,x.r);
console.log('FOURPLY MARGIN FINAL top 3 x 100');const f=c.slice(0,3).map(x=>({cfg:x.cfg,r:test(x.cfg,100,290000)})).sort(rank);for(const x of f)console.log('FINAL',x.cfg.name,x.r);console.log('WINNER',JSON.stringify(f[0].cfg),f[0].r);`;
