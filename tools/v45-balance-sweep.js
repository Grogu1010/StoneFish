...
function play(seed,modelWhite,useV45){const g=new Chess();const rng=seeded(seed);const old=Math.random;Math.random=rng;try{for(let p=0;p<1000;p++){const isModel=(g.turn()==='w')===modelWhite;const m=isModel?(useV45?getStonefishV45Move(g):getStonefishV4Move(g)):getStonefishV4Move(g);if(!m){if(!g.in_check())return'D';const ww=g.turn()==='b';return ww===modelWhite?'W':'L';}g._applyRaw(m._raw,true);if(cheapDraw(g))return'D';}return'D';}finally{Math.random=old;}}
...
console.log('control',JSON.stringify(test(false,1000,211000)));
console.log('production60',JSON.stringify(test(true,5000,221000)));
`;
new Function(src+'\n'+harness)();
