...
const profiles={
  ...
  flatish:{engine:0.55,mainline:0.58,rare:2.20,solid:0.90,dynamic:1.25,hypermodern:1.12},
  antiTheory:{engine:0.25,mainline:0.25,rare:4.50,solid:1.15,dynamic:1.35,tactical:0.95}
};
let ACTIVE={};
const originalWeight=stonefishV45ProfileWeight;
...
function candidate(g){let c=getStonefishV3BestRawMoves(g).slice();if(!c.length)return null;for(let i=0;i<STONEFISH_V4_ORDER.length&&c.length>1;i++){const k=STONEFISH_V4_ORDER[i];if(i===5){const b=stonefishV45BookMove(g,0,c);if(b)return stonefishV3PublicMove(g,b);}if(k==='mobility'&&c.length>1){c=stonefishV45BestByInverse(g,c,'oppCheckRisk');if(c.length>1)c=stonefishV45BestByInverse(g,c,'oppMobility');}c=stonefishV4BestByCriterion(g,c,k);if(i===11&&c.length>1)c=stonefishV45StrictPatternTieBreak(g,c);}return pub(g,c);}
