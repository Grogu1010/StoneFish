...
const tail=String.raw`const C=[];const base=${JSON.stringify(base)};function add(name,pos,e){const o=base.slice();o.splice(pos,0,'fourPly');C.push({name,order:o,developUntil:8,queenUntil:14,endgameMaterial:e});}
for(const e of [8,10,12,14]){add('hang_e'+e,4,e);add('queen_e'+e,5,e);}
console.log('FOURPLY PHASE SCREEN',C.length,'x 12');const s=C.map(c=>({cfg:c,r:test(c,12,161000)})).sort(rank);for(const x of s)console.log(x.cfg.name,x.r);
console.log('FOURPLY PHASE CONFIRM top 5 x 40');const c=s.slice(0,5).map(x=>({cfg:x.cfg,r:test(x.cfg,40,164000)})).sort(rank);for(const x of c)console.log(x.cfg.name,x.r);
console.log('FOURPLY PHASE FINAL top 3 x 100');const f=c.slice(0,3).map(x=>({cfg:x.cfg,r:test(x.cfg,100,170000)})).sort(rank);for(const x of f)console.log('FINAL',x.cfg.name,x.r);console.log('WINNER',JSON.stringify(f[0].cfg),f[0].r);`;
