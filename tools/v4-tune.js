...
function attackedMax(g,s){let m=0;for(let q=0;q<64;q++){const p=g.boardState[q];if(!p||(p>0?1:-1)!==s||Math.abs(p)===6)continue;if(oppAtk(g,s,q))m=Math.max(m,V[Math.abs(p)]||0);}return-m;}
function hangingTotal(g,s){let z=0;for(let q=0;q<64;q++){const p=g.boardState[q];if(!p||(p>0?1:-1)!==s||Math.abs(p)===6)continue;if(oppAtk(g,s,q)&&!ownDef(g,s,q))z+=V[Math.abs(p)]||0;}return-z;}
function defendedMat(g,s){let z=0;for(let q=0;q<64;q++){const p=g.boardState[q];if(!p||(p>0?1:-1)!==s||Math.abs(p)===6)continue;if(ownDef(g,s,q))z+=V[Math.abs(p)]||0;}return z;}
function queenSafe(g,s){for(let q=0;q<64;q++)if(g.boardState[q]===s*5)return oppAtk(g,s,q)?0:1;return 1;}
function majorSafe(g,s){let z=0;for(let q=0;q<64;q++){const p=g.boardState[q];if((p===s*5||p===s*4)&&!oppAtk(g,s,q))z++;}return z;}
function feature(g,m,c){if(c==='promotion')return m.promotion||0;if(c==='castleNow')return(m.flags&12)?1:0;if(c==='check')return g.fastGivesCheck(m)?1:0;if(c==='fiftyReset')return g.halfmove>=60&&(m.piece===1||m.captured)?1:0;g.fastApply(m);const s=-g.side;let z=0;if(c==='openingDevelop')z=g.fullmove<=13?dev(g,s):0;else if(c==='queenDiscipline'){if(g.fullmove<=10){const h=s===1?3:59;z=g.boardState[h]===s*5?1:0;}}else if(c==='repetitionAvoid')z=-(g.positionCounts.get(g.fastPositionKey())||0);else if(c==='endgamePawnProgress')z=pprog(g,s);else if(c==='endgameCheck')z=eg(g,s)&&g.in_check()?1:0;else if(c==='mobility')z=g.fastMobility(s);else if(c==='pieceSupport')z=ownDef(g,s,m.to)?1:0;else if(c==='kingFreedom')z=kfree(g,s);else if(c==='center')z=center(g,s);else if(c==='minorCentral')z=minor(g,s);else if(c==='kingProtection')z=kprot(g,s);else if(c==='pawnStructure')z=pawnStruct(g,s);else if(c==='rookActivity')z=rook(g,s);else if(c==='kingPlacement')z=kplace(g,s);else if(c==='boardControl')z=control(g,s);else if(c==='hangingMax')z=hangingMax(g,s);else if(c==='attackedMax')z=attackedMax(g,s);else if(c==='hangingTotal')z=hangingTotal(g,s);else if(c==='defendedMaterial')z=defendedMat(g,s);else if(c==='queenSafe')z=queenSafe(g,s);else if(c==='majorSafe')z=majorSafe(g,s);g.fastUndo();return z;}
...
const saf=['hangingMax','attackedMax','hangingTotal','defendedMaterial','queenSafe','majorSafe'];
const C=[];let id=0;function add(o){C.push({name:'h'+id++,order:o});}
add(base);
for(const x of saf)for(const pos of[1,2,3,4,5,8,9,10,11]){const o=base.slice();o.splice(pos,0,x);add(o);}
for(const combo of[['hangingMax','hangingTotal'],['hangingMax','queenSafe'],['hangingMax','majorSafe'],['hangingMax','defendedMaterial'],['attackedMax','hangingMax'],['queenSafe','hangingMax'],['majorSafe','hangingMax'],['hangingMax','hangingTotal','defendedMaterial']])for(const pos of[1,2,3,5,8,9]){const o=base.slice();o.splice(pos,0,...combo);add(o);}
console.log('HANGING SCREEN',C.length,'x 12');const s=C.map(cfg=>({cfg,r:test(cfg,12,0)})).sort(rank);for(const x of s.slice(0,15))console.log(x.cfg.name,x.cfg.order.join('>'),x.r);
console.log('CONFIRM top 12 x 40');const c=s.slice(0,12).map(x=>({cfg:x.cfg,r:test(x.cfg,40,1700)})).sort(rank);for(const x of c)console.log(x.cfg.name,x.cfg.order.join('>'),x.r);
console.log('FINAL top 5 x 100');const f=c.slice(0,5).map(x=>({cfg:x.cfg,r:test(x.cfg,100,8000)})).sort(rank);for(const x of f)console.log('FINAL',x.cfg.name,x.cfg.order.join('>'),x.r);console.log('WINNER',JSON.stringify(f[0].cfg),f[0].r);
`;
new Function(engine+'\n'+v3+'\n'+harness)();
