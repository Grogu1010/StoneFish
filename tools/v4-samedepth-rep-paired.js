...
const R=['promotion','castleNow','openingDevelop','hangingMax','rep2Lead3','mobility','queenDiscipline','repetitionAvoid','fiftyReset','endgamePawnProgress','endgameCheck','pieceSupport','kingFreedom','center','minorCentral','kingProtection','pawnStructure','rookActivity','kingPlacement','boardControl'];
const control={name:'control',order:M},guard={name:'rep2Lead3',order:R};
const starts=[90000,740000,820000,860000];
const matrix={};let changed=0;for(const st of starts){const block={};for(let i=0;i<100;i++){const seed=st+i,a=play(control,seed),b=play(guard,seed),k=a+'>'+b;matrix[k]=(matrix[k]||0)+1;block[k]=(block[k]||0)+1;if(a!==b){changed++;console.log('CHANGE',seed,k);}}console.log('BLOCK',st,block);}
