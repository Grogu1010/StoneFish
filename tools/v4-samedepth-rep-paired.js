...
function feature(g,m,c){if(c==='rep2Lead2_5')return repByLead(g,m,1,2.5);`);
...
const R=['promotion','castleNow','openingDevelop','hangingMax','rep2Lead2_5','mobility','queenDiscipline','repetitionAvoid','fiftyReset','endgamePawnProgress','endgameCheck','pieceSupport','kingFreedom','center','minorCentral','kingProtection','pawnStructure','rookActivity','kingPlacement','boardControl'];
const ctrlCfg={name:'control',order:M},guardCfg={name:'rep2Lead2_5',order:R};
const starts=[90000,740000,820000,860000];
const matrix={};let changed=0;for(const st of starts){const block={};for(let i=0;i<100;i++){const seed=st+i,a=play(ctrlCfg,seed),b=play(guardCfg,seed),k=a+'>'+b;matrix[k]=(matrix[k]||0)+1;block[k]=(block[k]||0)+1;if(a!==b){changed++;console.log('CHANGE',seed,k);}}console.log('BLOCK',st,block);}
console.log('MATRIX',matrix,'changed',changed);
`;
