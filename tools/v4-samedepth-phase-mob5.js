...
src=src.replace("function feature(g,m,c){",String.raw`function feature(g,m,c){if(c==='mobilityAfter5'){const active=g.fullmove>5;g.fastApply(m);const s=-g.side,z=active?g.fastMobility(s):0;g.fastUndo();return z;}`);
...
const base=['promotion','castleNow','openingDevelop','hangingMax','queenDiscipline','repetitionAvoid','fiftyReset','endgamePawnProgress','endgameCheck','mobilityAfter5','pieceSupport','kingFreedom','center','minorCentral','kingProtection','pawnStructure','rookActivity','kingPlacement','boardControl'];
...
console.log('SAMEDEPTH PHASE+MOB5 SCREEN',C.length,'x 12');const s=C.map(cfg=>({cfg,r:test(cfg,12,640000)})).sort(rank);for(const x of s.slice(0,25))console.log(x.cfg.name,x.r);
console.log('SAMEDEPTH PHASE+MOB5 CONFIRM top 16 x 40');const c=s.slice(0,16).map(x=>({cfg:x.cfg,r:test(x.cfg,40,645000)})).sort(rank);for(const x of c)console.log(x.cfg.name,x.r);
console.log('SAMEDEPTH PHASE+MOB5 FINAL top 6 x 100');const f=c.slice(0,6).map(x=>({cfg:x.cfg,r:test(x.cfg,100,655000)})).sort(rank);for(const x of f)console.log('FINAL',x.cfg.name,x.r,JSON.stringify(x.cfg));console.log('WINNER',JSON.stringify(f[0].cfg),f[0].r);
`;
