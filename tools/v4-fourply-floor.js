...
const tail=String.raw`const C=[];const base=${JSON.stringify(base)};for(const f of[-5,-3,-2,-1,0,1,2,3,5]){const o=base.slice();o.splice(5,0,'fourPlyFloor:'+f);C.push({name:'f'+f,order:o,developUntil:8,queenUntil:14,endgameMaterial:14});}
console.log('FOURPLY FLOOR SCREEN',C.length,'x 12');const s=C.map(c=>({cfg:c,r:test(c,12,211000)})).sort(rank);for(const x of s)console.log(x.cfg.name,x.r);
console.log('FOURPLY FLOOR CONFIRM top 5 x 40');const c=s.slice(0,5).map(x=>({cfg:x.cfg,r:test(x.cfg,40,214000)})).sort(rank);for(const x of c)console.log(x.cfg.name,x.r);
console.log('FOURPLY FLOOR FINAL top 3 x 100');const f=c.slice(0,3).map(x=>({cfg:x.cfg,r:test(x.cfg,100,220000)})).sort(rank);for(const x of f)console.log('FINAL',x.cfg.name,x.r);console.log('WINNER',JSON.stringify(f[0].cfg),f[0].r);`;
