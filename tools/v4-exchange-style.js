...
source = source.replace(
  ...
  "function feature(g,m,c){if(c==='promotion')return m.promotion||0;if(c==='castleNow')return(m.flags&12)?1:0;if(c==='check')return g.fastGivesCheck(m)?1:0;if(c==='captureNow')return V[m.captured]||0;if(c==='quietMove')return -(V[m.captured]||0);if(c==='lowValueCaptor')return m.captured?-(V[m.piece]||0):0;if(c==='fiftyReset')return g.halfmove>=60&&(m.piece===1||m.captured)?1:0;"
);
...
add('base',base.slice());
for(const feat of ['captureNow','quietMove','lowValueCaptor']){
  for(const pos of [3,4,5,8,9,10,11,12,13]){
    const o=base.slice();o.splice(pos,0,feat);add(feat+'_p'+pos,o);
  }
}
for(const seq of [
 ['hangingMax','captureNow','lowValueCaptor'],
 ['hangingMax','lowValueCaptor','captureNow'],
 ['hangingMax','quietMove','pieceSupport'],
 ['hangingMax','pieceSupport','quietMove'],
 ['hangingMax','captureNow','pieceSupport'],
 ['hangingMax','pieceSupport','captureNow']
]){
...
console.log('EXCHANGE STYLE SCREEN',C.length,'x 20');
const s=C.map(cfg=>({cfg,r:test(cfg,20,42000)})).sort(rank);
for(const x of s.slice(0,12))console.log(x.cfg.name,x.cfg.order.join('>'),x.r);
console.log('EXCHANGE STYLE CONFIRM top 8 x 60');
const c=s.slice(0,8).map(x=>({cfg:x.cfg,r:test(x.cfg,60,44000)})).sort(rank);
for(const x of c)console.log(x.cfg.name,x.cfg.order.join('>'),x.r);
console.log('EXCHANGE STYLE FINAL top 4 x 100');
const f=c.slice(0,4).map(x=>({cfg:x.cfg,r:test(x.cfg,100,48000)})).sort(rank);
