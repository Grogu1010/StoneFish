...
// SAME DEPTH ONLY: history + current/resulting static material + move metadata.
...
function fiftyByLead(g,m,clockMin,leadMin){
  if(g.halfmove<clockMin)return 0;
  g.fastApply(m);const s=-g.side,lead=npm(g,s)-npm(g,-s);g.fastUndo();
  if(lead<leadMin)return 0;
  return (m.piece===1||m.captured)?1:0;
}
function feature(g,m,c){
  if(c==='rep2Lead2_5')return repByLead(g,m,1,2.5);
  if(c.startsWith('fiftyLead@')){const a=c.split('@');return fiftyByLead(g,m,Number(a[1]),Number(a[2]));}
`);
...
for(const clock of[60,70,80,90])for(const lead of[2.5,3,5])add('rep_f'+clock+'_l'+lead,['rep2Lead2_5','fiftyLead@'+clock+'@'+lead]);
...
console.log('REP+FIFTY',C.length,'variants x 4 blocks');
