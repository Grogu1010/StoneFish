...
const labels={4:'afterHanging',5:'afterQueen',6:'afterRepetition',7:'afterFifty',8:'afterPawnProgress',9:'afterEndgameCheck',10:'afterMobility'};
const tail=String.raw`const C=[];const base=${JSON.stringify(base)},labels=${JSON.stringify(labels)};for(const pos of[4,5,6,7,8,9,10]){const o=base.slice();o.splice(pos,0,'fourPly');C.push({name:labels[pos],order:o,developUntil:8,queenUntil:14,endgameMaterial:14});}
console.log('FOURPLY FINE SCREEN',C.length,'x 16');const s=C.map(c=>({cfg:c,r:test(c,16,181000)})).sort(rank);for(const x of s)console.log(x.cfg.name,x.r);
console.log('FOURPLY FINE CONFIRM top 5 x 50');const c=s.slice(0,5).map(x=>({cfg:x.cfg,r:test(x.cfg,50,184000)})).sort(rank);for(const x of c)console.log(x.cfg.name,x.r);
console.log('FOURPLY FINE FINAL top 3 x 100');const f=c.slice(0,3).map(x=>({cfg:x.cfg,r:test(x.cfg,100,190000)})).sort(rank);for(const x of f)console.log('FINAL',x.cfg.name,x.r);console.log('WINNER',JSON.stringify(f[0].cfg),f[0].r);`;
