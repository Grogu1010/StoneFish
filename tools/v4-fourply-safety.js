...
source=source.replace("function feature(g,m,c){",String.raw`function fourPlyScore(g,root){
  ...
  for(const reply of replies){
    ...
    if(!responses.length&&g.in_check()){
      g.fastUndo();
      if(-M<worst)worst=-M;
      continue;
    }
    ...
    for(const response of responses){
      ...
      let score;
      if(!fourth.length&&g.in_check()){
        score=M;
      }else{
        let fourthGain=0, fourthMate=false;
        for(const fm of fourth){
          const cap=V[fm.captured]||0;
          if(cap>fourthGain)fourthGain=cap;
          if(!fourthMate&&g.fastGivesCheck(fm)){
            g.fastApply(fm);
            if(g.fastMoves().length===0&&g.in_check())fourthMate=true;
            g.fastUndo();
          }
        }
...
const tail=String.raw`const C=[];const base=${JSON.stringify(base)};function cfg(name,pos){const o=base.slice();o.splice(pos,0,'fourPly');return{name,order:o,developUntil:8,queenUntil:14,endgameMaterial:14};}
C.push(cfg('afterDevelop',3),cfg('afterHanging',4),cfg('afterQueen',5),cfg('beforeMobility',9),cfg('afterMobility',10));
console.log('FOURPLY SCREEN',C.length,'x 8');const s=C.map(c=>({cfg:c,r:test(c,8,136000)})).sort(rank);for(const x of s)console.log(x.cfg.name,x.r);
console.log('FOURPLY CONFIRM top 3 x 30');const c=s.slice(0,3).map(x=>({cfg:x.cfg,r:test(x.cfg,30,139000)})).sort(rank);for(const x of c)console.log(x.cfg.name,x.r);
console.log('FOURPLY FINAL top 2 x 100');const f=c.slice(0,2).map(x=>({cfg:x.cfg,r:test(x.cfg,100,143000)})).sort(rank);for(const x of f)console.log('FINAL',x.cfg.name,x.r);console.log('WINNER',JSON.stringify(f[0].cfg),f[0].r);`;
