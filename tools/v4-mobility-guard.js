...
const tail=String.raw`const C=[];const base=${JSON.stringify(base)};for(const gap of [0,1,2,3,4,5]){const o=base.slice();o.splice(o.indexOf('mobility'),0,'mobilityGuard:'+gap);C.push({name:'g'+gap,order:o,developUntil:8,queenUntil:14,endgameMaterial:14});}
console.log('MOBILITY GUARD SCREEN',C.length,'x 10');const s=C.map(c=>({cfg:c,r:test(c,10,181000)})).sort(rank);for(const x of s)console.log(x.cfg.name,x.r);
console.log('MOBILITY GUARD CONFIRM top 4 x 30');const c=s.slice(0,4).map(x=>({cfg:x.cfg,r:test(x.cfg,30,184000)})).sort(rank);for(const x of c)console.log(x.cfg.name,x.r);
console.log('MOBILITY GUARD FINAL top 2 x 100');const f=c.slice(0,2).map(x=>({cfg:x.cfg,r:test(x.cfg,100,190000)})).sort(rank);for(const x of f)console.log('FINAL',x.cfg.name,x.r);console.log('WINNER',JSON.stringify(f[0].cfg),f[0].r);`;
