...
function loadEnginesGlobal() {
  ...
  return { v4: window.StonefishV4, v45: window.StonefishV45, v5f: window.StonefishV5Flash, v5: window.StonefishV5, v6f: window.StonefishV6Flash, v6: window.StonefishV6 };
}
...
function workerMatch(aName, bName, games = 2, startIndex = 0) {
  ...
  return { aName, bName, games, totals, ms, reasons, perGame };
}
...
function match(aName, bName, games = 4) {
  ...
  return scaleTwoGameChunk(aName, bName, games);
}

const args = process.argv.slice(2);
if (args.length >= 2) console.log(JSON.stringify(match(args[0], args[1], Number(args[2] || 4)), null, 2));
else for (const [a,b] of [['v6f','v5'],['v6','v6f'],['v6','v5']]) console.log(JSON.stringify(match(a,b, Number(args[0] || 4)), null, 2));
