...
function applyPublic(game, move) {
...
function applyOpening(game, opening) {
  for (const uci of opening.moves) {
    if (!game.move(decodeUci(uci))) throw new Error(`Illegal opening move ${uci} in ${opening.name}`);
  }
}
...
function simulate(currentEngine, referenceEngine, opening, currentIsWhite, seed, maxPlies = 600) {
  ...
  while (!currentGame.game_over() && plies < maxPlies) {
    ...
    const a = applyPublic(currentGame, move);
    const b = applyPublic(referenceGame, move);
    if (!a || !b) {
      return {
        result: currentTurn ? 'loss' : 'win',
...
const summary = {
  ...
  draws: totals.draw,
  score: (totals.win + totals.draw * 0.5) / 2,
  averagePlies: totalPlies / 2,
  reasons
};
console.log('\nSTONEFISH_V5_PRO_OPENING_SUITE ' + JSON.stringify(summary));

if (process.env.H2H_RESULT_FILE) fs.writeFileSync(process.env.H2H_RESULT_FILE, JSON.stringify(summary));
