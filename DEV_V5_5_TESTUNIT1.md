...
The reply policy learns which quiet moves the opponent selects from the available alternatives. Its 13 geometric preference weights start at zero every game. After four voluntary quiet choices, a frozen snapshot guides quiet opponent move ordering. Low-priority quiet replies may receive a reduced null-window probe; the host verifies at full depth whenever that probe favors the opponent. Captures, promotions, king moves and check positions do not receive this reduction. Before each learning update, ARMX measures how surprising the actual quiet choice was relative to uniform selection. An exponentially weighted average of this prediction quality requests between 1,200 and 4,800 search nodes, spending more when the opponent is unpredictable. These notes reset every game. The same prediction-surprise notes now request a depth limit between four and six plies within that node budget. No ARMX retains the base evaluation, four-ply limit and 1,200-node request. Both variants skip repetition lookups before eight reversible plies, while retaining all path history. Native position keys use a flat 64-character board string with exactly the original repetition identity, avoiding the cost of repeated concatenation. The key contract covers 1,002 capture-generation positions and a complete speculative repetition cycle. Bounded finalist adjustments remain active.
