...
The two Developer-menu models share the same native host and evaluation. Only '5.5(testunit1)' invokes the separate ARMX-preview opponent model. Its learned reply preferences can influence search and therefore finalist scores. No ARMX supplies no reply policy and retains its previous behavior. Neither model is released in the normal opponent selector.
...
The reply policy learns which quiet moves the opponent selects from the available alternatives. Its 13 geometric preference weights start at zero every game. After four voluntary quiet choices, a frozen snapshot guides quiet opponent move ordering. Low-priority quiet replies may receive a reduced null-window probe; the host verifies at full depth whenever that probe favors the opponent. Captures, promotions, king moves and check positions do not receive this reduction. Before each learning update, ARMX measures prediction surprise relative to uniform selection. Its per-game notes request 1,200 base nodes plus up to 3,600 when predictions miss and up to 4,800 as voluntary quiet observations accumulate (full allowance at eight). There is no policy until four such observations. An active policy permits depth six. No ARMX stays at 1,200 nodes and depth four with the original decisions. Both variants share stable priority ordering, an exactly restored material counter, compact collision-free repetition keys, and compiled evaluation/legal-move helpers with an identical JavaScript fallback. Public game history keys remain unchanged. The compiled module has no imports, opponent state or search policy. See native/README.md for source and rebuild instructions. Bounded finalist adjustments remain active.
...
The new evidence-effort build scored **72W-23L-5D /100** against No ARMX and **74W-21L-5D /100** on the second opening set: **146 wins /200 (73%)**, up from 116/200. The integrated build scored **93W-4L-3D /100** against Pro. Its isolated speed was **2.052x** faster than Pro, below the required 3x, so it remains a development-only test unit. Do not release or remove either testunit1 name yet. Notes still reset after every game, and the original No-ARMX control reproduces exact moves, scores, depth and node counts. No blog.

The two contribution runs used the prototype, with 50 mirrored varied opening pairs each. Integration reproduces all 128 saved prototype positions. The Pro run used the integrated code. Evidence is in evidence-effort-100.json.gz, evidence-effort-holdout-100.json.gz, evidence-effort-pro-100.json.gz and evidence-effort-golden.json.gz under benchmarks/v5_5/. Full integrated release confirmation on fresh openings remains pending. The compiler helper contract also compares the original JavaScript fallback, checks 5,524 positions and all 2,080 packed metadata combinations, and audits material counts through search.

### Previous adaptive-horizon checkpoint

The adaptive-horizon build scored **57W-34L-9D /100** against No ARMX and **59W-31L-10D /100** on the second, non-overlapping opening set: **116W-65L-19D /200 (58% actual wins)**. This improves the preceding adaptive-effort build's 112/200. Against Pro it scored **88W-9L-3D /100**, with **3.249x** faster engine-only time per move in an isolated run. No ARMX retains exact score, move, depth and node-count parity with the original control; its latest full confirmation was 92W-7L-1D /100 against Pro. The 65/100 contribution target remains unmet. Keep both testunit1 names and development-only availability; do not release or write a blog. These full games tested the prototype; the integrated code exactly matches 128 saved prototype positions, including all learned state and the control. Integrated confirmation reproduced 57W-34L-9D vs No ARMX, 88W-9L-3D vs Pro (3.247x faster), and 92W-7L-1D No ARMX vs Pro. Full records are in adaptive-horizon-integrated-100.json.gz. Only the ARMX-vs-Pro timing phase was isolated.

Evidence: `adaptive-horizon-100.json.gz`, `adaptive-horizon-holdout-100.json.gz`, and `adaptive-horizon-pro-100.json.gz` in `benchmarks/v5_5/`. `adaptive-horizon-golden.json.gz` preserves the tested source and parity fixtures.

### Previous adaptive-effort checkpoint
...
### Previous learned-reply checkpoint
...
Prequential audits predict each opponent choice before updating that game's model. On 3,379 eligible quiet choices from the first 100-game set, the current geometric model placed the played move among its top three about **32.0%** of the time; contextual safety measured after the move improved that to **38.8%**. However, its game screen remained at **20 wins /40**, equal to the current build. Better prediction alone is not sufficient evidence to promote a chess engine. The audits and their source are included in the archive. Those follow-ups did not replace the engine from commit `0777b0a70e81834d5bf3da9588701fb499f0ca8f`.
