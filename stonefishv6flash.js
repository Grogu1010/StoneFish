(function () {
  ...
  function allOpeningLines(openingBook, color) {
    ...
    const add = (opening, sideWeight, bookSide) => {
      if (openingBook.isOpeningAllowed && !openingBook.isOpeningAllowed(opening, bookSide, 6)) return;
  ...
  window.StonefishV6Flash = {
    name: "Stonefish v6 Flash",
    ...
    chooseMove(game, color) {
      ...
      if (bestBook) {
        const danger = quickDangerProfile(game, bestBook.testState, color, false);
      ...
      for (let i = 0; i < candidates.length; i++) {
        ...
        const danger = quickDangerProfile(game, candidate.testState, color, false);
        candidate.danger = danger;
        candidate.score -= Math.max(0, danger.worstNetLoss) * 3.05;
        candidate.score -= danger.totalLoss * 0.95;
        if (danger.worstNetLoss >= 300) candidate.score -= 420;
        if (danger.mateDanger) candidate.score -= MATE_SCORE / 2;
        if (i < 2 || candidate.book || danger.worstNetLoss >= 240 || danger.mateDanger) {
          const opponentPlan = quickOpponentPlanProfile(game, candidate.testState, color);
          candidate.opponentPlan = opponentPlan;
          candidate.score -= opponentPlan.worstNetPlan * 0.58;
          candidate.score -= opponentPlan.totalNetPlan * 0.10;
          candidate.score += Math.max(0, currentPlan.worstNetPlan - opponentPlan.worstNetPlan) * 0.52;
          if (opponentPlan.forcingPlans >= 3) candidate.score -= 32;
          if (opponentPlan.bestPreparedness >= opponentPlan.bestRawPlan * 0.85 && opponentPlan.bestRawPlan > 170) candidate.score += 22;
          if (candidate.book && opponentPlan.worstNetPlan > currentPlan.worstNetPlan + 150) candidate.score -= Math.min(220, opponentPlan.worstNetPlan - currentPlan.worstNetPlan);
          if (opponentPlan.matePlan && opponentPlan.worstNetPlan > 220) candidate.score -= MATE_SCORE / 6;
        }
    ...
    _stonefishDepthProfile: "v6 fast architecture with v5-level refutation depth, opponent-plan preparedness scoring, all-candidate danger checks, book ordering, and lighter scoring"
  };
})();
