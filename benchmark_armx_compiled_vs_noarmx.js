// Direct quality/speed comparison: compiled full-effort ARMX candidate vs No-ARMX.
// Both sides use the same current model and per-game learning; only the candidate routes the same full ARMX request through the compiled search core.
const fs = require('fs');
const vm = require('vm');
...
function withFastMode(enabled,fn){
  const had=Object.prototype.hasOwnProperty.call(process.env,'ARMX_COMPILED_EXPERIMENT'),old=process.env.ARMX_COMPILED_EXPERIMENT;
  if(enabled)process.env.ARMX_COMPILED_EXPERIMENT='1';else delete process.env.ARMX_COMPILED_EXPERIMENT;
  try{return fn();}finally{if(had)process.env.ARMX_COMPILED_EXPERIMENT=old;else delete process.env.ARMX_COMPILED_EXPERIMENT;}
}
...
function currentMove(game){return withFastMode(false,()=>getStonefishV55NoARMXMove(game));}
