...
effortModel.qualitySum = -1;
assert.equal(armxPreviewOpponentPolicy(knights, 1).searchBudget, ARMX_PREVIEW.maxSearchNodes);
...
for (const row of golden.positions) {
  ...
  else if (row.armx.entries.length) assert.ok(armxResult.entries.length > 0);
  else assert.equal(armxResult.entries.length, 0);
