// Diagnose the first move where the fast candidate disagrees with released v5 Pro.
// This is diagnostic only: normal legal positions, no outcome manipulation.
...
function makeEngine(dir, includeGeometry) {
  const ctx = vm.createContext({ console });
  const files = baseFiles.slice();
  if (includeGeometry && fs.existsSync(path.join(dir, 'Stonefish_v5_pro_geometry_patch.js'))) {
...
for (let ply = 0; ply < 160 && !currentGame.game_over(); ply += 1) {
  ...
  if (currentTurn) {
    ...
    if (key(candidate) !== key(oldSuggestion)) {
      ...
      let oldRank = -1;
      const rows = scored.slice(0, 8).map((entry, index) => {
        const uci = current.__uci(currentGame, entry.raw);
        if (uci === oldKey) oldRank = index + 1;
        return {
          rank: index + 1,
          move: uci,
          total: entry.score,
      ...
      if (oldRank < 0) {
        const index = scored.findIndex(entry => current.__uci(currentGame, entry.raw) === oldKey);
        oldRank = index >= 0 ? index + 1 : -1;
      }
      console.log('STONEFISH_V5_PRO_DIVERGENCE ' + JSON.stringify({
        ...
        releasedSuggestionRank: oldRank,
        top: rows
      }));
      process.exit(0);
    }
